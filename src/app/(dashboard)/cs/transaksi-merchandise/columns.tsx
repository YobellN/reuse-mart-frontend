...
import KonfirmasiKlaimDialog from "./konfirmasi-klaim-dialog";
import { handleKlaimMerchandise } from "@/services/transaksi_merchandise/transaksi-merchandise-service";
...
export const columns: ColumnDef<TransaksiMerchandise>[] = [
...

  {
    ...
    cell: ({ row }) => {
      return (
        <KonfirmasiKlaimDialog
          id={row.original.id_transaksi_merchandise}
          onKlaim={() =>
            handleKlaimMerchandise(row.original.id_transaksi_merchandise)
          }
          label={row.original.merchandise.nama_merchandise}
          detail={row.original.pembeli.user.nama}
        />
      );
    },
  },
