...
export default function DisplayProdukSearch() {
  ...
  async function loadPage(p: number) {
    ...
    setLoading(true);
    try {
      const { data, meta }: Paginated<Produk> = await getProduk({
        limit: 6,
        page: p,
      });
  ...
  return (
    <div className="my-6 bg-white rounded-xl p-3 lg:p-6 border border-gray-200 dark:bg-slate-950 dark:border-slate-600">
      ...
      <div className="grid gap-2 md:gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5">
        {items.map((item) => (
