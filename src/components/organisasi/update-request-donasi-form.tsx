...
import { RequestDonasi, RequestDonasiFormSchema, RequestDonasiSchema } from "@/services/organisasi/schema-organisasi"
import { handleUpdateRequestDonasi } from "@/services/organisasi/organisasi-services"
import { Textarea } from "../ui/textarea"


export default function UpdateRequestDonasiForm({requestDonasi}: {requestDonasi: RequestDonasi }) {
    ...
    async function handleSubmit(data: FormData) {
        ...
        try {
            const res = await handleUpdateRequestDonasi(data, requestDonasi.id_request_donasi);
            ...
            if (res.message === "Request donasi berhasil diubah") {
                ...
                setSubmit(false);
                toast.error("Gagal mengubah request donasi");
            }
        } catch (err) {
            setSubmit(false);
    ...
    return (
        <Card className="max-w-2xl mx-auto mt-10 shadow-md">
            <CardHeader>
                <CardTitle className="text-2xl text-center">Edit Request Donasi</CardTitle>
            </CardHeader>
            <CardContent>
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="pt-4">
                            <AlertDialog open={open} onOpenChange={setOpen}>
                                <Button type="submit" className="w-full" disabled={submit}>{submit ? "Memproses..." : "Edit Request"}</Button>
                                <AlertDialogContent>
                                    <AlertDialogHeader>
