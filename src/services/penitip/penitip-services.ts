...
export async function getPenitipById(id_penitip: string): Promise<Penitip | null> {
    try {
        const res = await api.get(`/penitip/${id_penitip}`);
...
export async function handleNewPenitip(formData: FormData): Promise<IResponse<Penitip>> {
    try {
      const res = await api.post("/penitip", formData, { headers: { "Content-Type": "multipart/form-data" } });
      return {
...
export default async function handleUpdatePenitip(formData: FormData, id_penitip: string): Promise<IResponse<Penitip>> {
    try {
      ...
      const res = await api.post(`/penitip/${id_penitip}`, formData, {
...
export async function handleDeletePenitip(id: string) : Promise<IResponse<any>>{
    try {
        const res = await api.delete(`/penitip/${id}`);
        return {
            message: res.data.message,
            data: res.data.data
        }
