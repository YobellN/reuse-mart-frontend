...
export async function getProdukTitipanGudang(status_produk?: string): Promise<ProdukTitipan[]> {
  try {
...
export async function pengambilanProdukTitipan(id_produk: string): Promise<IResponse<any>> {
  try {
    const res = await api.patch(`/penitipan/pengambilan-produk-titipan/${id_produk}`);
    return {
      message: res.data.message,
      data: res.data.data
    }
